package com.example.lyricsflowfw.core.controller;
...
public abstract class BaseFlashcardController<F extends BaseFlashcard> {

    protected final BaseFlashcardService<F> flashcardService;

    protected BaseFlashcardController(BaseFlashcardService<F> flashcardService) {
    ...
    public ResponseEntity<?> getAllFlashcards(@RequestParam Long userId) {
        try {
            List<F> flashcards = this.flashcardService.getAllFlashcardsByUserId(userId);
